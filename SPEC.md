...
| model resolution          | Inference is **explicit**. The default provider is **`boardwalk`** (managed inference): an `agent()` call that names no provider/model goes to the managed lane, which works when `BOARDWALK_API_KEY` is set (gateway URL via `BOARDWALK_INFERENCE_URL`/config, default `api.boardwalk.sh`), else a clear error. Naming a provider is always honored: a built-in (`anthropic`/`openai`/`google`, key from `ANTHROPIC_API_KEY`/… — never auto-selected) or an `inference.providers.<name>.{base_url, api_key_env}` endpoint, including a **local** OpenAI-compatible server (Ollama, vLLM). `default_model` sets the omitted-model fallback                                                                                                                                                                                 |
